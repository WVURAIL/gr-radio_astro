...
class nsf_detect_80_web(gr.top_block):
    def __init__(self):
        ...
        # Blocks
        ##################################################
        self.nsigma_textbox = bokehgui.textbox(self.widget_lst, str(nsigmas), 'N Sigma' +": ")
        self.nsigma_textbox.add_callback(
          lambda attr, old, new: self.set_nsigma(eng_notation.str_to_num(new)))
        ...
        _EventMode_choices = {"[0]": True, "[]": False}
        _EventMode_choices_inv = dict((_EventMode_choices[k],[0] if k == "[0]" else []) for k in _EventMode_choices.keys())
        self.EventMode_checkbox = bokehgui.checkbox(self.widget_lst, _EventMode_choices_inv[True], 'Event Write' + ": ", inline = True)
        self.EventMode_checkbox.add_callback(
          lambda new: self.set_EventMode(bool(_EventMode_choices[str(new)])))
        ...
        self.Detect_radiobutton = bokehgui.radiobutton(self.widget_lst, None, self._Detect_labels, inline = True)
        self.Detect_radiobutton.add_callback(
                  lambda new: self.set_Detect(int(self._Detect_options[new])))
    ...
    def set_nsigmas(self, nsigmas):
        ...
        self.nsigma_textbox.set_value(self.nsigmas)
...
def main(top_block_cls=nsf_detect_80_web, options=None):
