...
class nsf_detect_100(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Nsf Airspy Event Detect: 10 MHz", catch_exceptions=True)
        ...
        self.settings = Qt.QSettings("GNU Radio", "nsf_detect_100")
        ...
        self.top_grid_layout.addWidget(self._Gain3_tool_bar, 2, 6, 1, 1)
        for r in range(2, 3):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(6, 7):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.radio_astro_detect_0 = radio_astro.detect(fftsize, nsigma, Frequency, Bandwidth, fftsize/Bandwidth, int(Mode))
        ...
        self.top_grid_layout.addWidget(self._qtgui_time_sink_x_0_0_win, 3, 2, 5, 5)
        for r in range(3, 8):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(2, 7):
            self.top_grid_layout.setColumnStretch(c, 1)
    ...
    def set_Mode(self, Mode):
        self.Mode = Mode
        self._Mode_callback(self.Mode)
        self.radio_astro_detect_0.set_mode(int(self.Mode))
...
def main(top_block_cls=nsf_detect_100, options=None):
