...
class nsf_watch_80_no_gui(gr.top_block):

    def __init__(self):
        gr.top_block.__init__(self, "Nsf Detect+Spectra: SDRPlay 8MHz Astronomical Obs.", catch_exceptions=True)
        ...
        self.radio_astro_ra_ascii_sink_0 = radio_astro.ra_ascii_sink(ObsName+".not", observer, fftsize, Frequency, Bandwidth, Azimuth, Elevation, 1, 0, 4**5, nAve, telescope_save, device_save, float(IF_attn), float(Gain2), float(Gain1))
...
def main(top_block_cls=nsf_watch_80_no_gui, options=None):
    tb = top_block_cls()
