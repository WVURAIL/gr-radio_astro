...
class nsf_detect_80(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "NSF Detect 8 MHz SDRPlay", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("NSF Detect 8 MHz SDRPlay")
        ...
        self.settings = Qt.QSettings("GNU Radio", "nsf_detect_80")
        ...
        self._EventMode_choices_inv = dict((v,k) for k,v in self._EventMode_choices.items())
        ...
        self._Detect_options = [0, 1]
        # Create the labels list
        self._Detect_labels = ['Monitor', 'Detect']
        ...
        self._Detect_button_group = variable_chooser_button_group()
        self._Detect_group_box.setLayout(self._Detect_box)
        for i, _label in enumerate(self._Detect_labels):
            radio_button = Qt.QRadioButton(_label)
            self._Detect_box.addWidget(radio_button)
            self._Detect_button_group.addButton(radio_button, i)
        self._Detect_callback = lambda i: Qt.QMetaObject.invokeMethod(self._Detect_button_group, "updateButtonChecked", Qt.Q_ARG("int", self._Detect_options.index(i)))
        self._Detect_callback(self.Detect)
        self._Detect_button_group.buttonClicked[int].connect(
            lambda i: self.set_Detect(self._Detect_options[i]))
        self.top_grid_layout.addWidget(self._Detect_group_box, 7, 0, 1, 2)
        for r in range(7, 8):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(self._qtgui_time_sink_x_0_0_win, 3, 3, 4, 6)
        for r in range(3, 7):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(3, 9):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.top_grid_layout.addWidget(self._qtgui_histogram_sink_x_0_win, 5, 0, 2, 3)
        for r in range(5, 7):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(0, 3):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.connect((self.sdrplay3_rsp1a_0, 0), (self.blocks_stream_to_vector_0_0_0, 0))
...
def main(top_block_cls=nsf_detect_80, options=None):
