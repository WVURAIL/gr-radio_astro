...
* The first bench test gives two outputs. ![Figure 1](Simulated_pulse%20_to_sink_flowgraph.png)
* One is difference of the sources and the other is one source untouched.  ![Figure 2](Simulated_pulse%20_to_sink.png)
...
* With the PFB enabled, there is a phase shift introduced. This results in packets not being exactly aligned seen by the difference of the two. ![Figure 5](Simulated%20pulse_to_Spectrometer_to_sink_PFB_enabled.png)
...
`Full_detection_Flowgraph.grc`, `Noise_collection.grc`,
`Pulsar_file_detection.grc` and `bench_testing.grc` all carry
`created='3.7.11'`. Current GNU Radio Companion releases can attempt to convert
that XML. With GNU Radio 3.10.9.2, `bench_testing.grc` converts with deprecated
...
supported examples. The generated `top_block.py` is likewise a Python 2-era
...
See the [branch history](../BRANCH_HISTORY.md) for the full 2026 branch audit.
